    ...
    def single_choose(self, agents, client, G, pokemons):
        if self.change:
            ...
            for p in pokemons:
                ...
                path = path + list(between_these)

                # path = [agents[0].src, between_these[0], between_these[1]]
            ...
            self.path = nx.algorithms.approximation.traveling_salesman_problem(G=self.DG, nodes=path, cycle=True)

        # Path as in path to reach pokemon, if it is not empty
        if len(self.path) > 0 and agents[0].dest == -1:
            client.choose_next_edge(
                '{"agent_id":' + str(0) + ', "next_node_id":' + str(self.path.pop(0)) + '}')

        elif len(self.path) == 0 and len(pokemons) > 0:
            self.path = nx.approximation.traveling_salesman_problem(G=self.DG, nodes=[agents[0].src] + list(self.find_nodes(pokemons[0])), cycle=True)
